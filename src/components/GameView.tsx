...
import { speak, initTTS, warmUpTTS, prefetchSpeech } from '../utils/tts';
...
export default function GameView() {
  ...
  useEffect(() => {
    initTTS(); // start loading the Android voice list before the first word
  }, []);

  useEffect(() => {
    // Прогреваем аудио первых двух слов ещё до нажатия TAP TO START
    if (!wordSet || words.length === 0) return;
    const isN2T = mode === 'written_native_to_target' || mode === 'audio_native_to_target';
    const lang = isN2T ? wordSet.nativeLang : wordSet.targetLang;
    words.slice(0, 2).forEach((w) => prefetchSpeech(isN2T ? w.native : w.target, lang));
  }, [words, wordSet, mode]);
  ...
  const loadTurn = async (index: number) => {
    ...
    // Пока играется текущий ход, заранее подгружаем аудио следующего слова
    const nextWord = words[index + 1];
    if (nextWord) {
      const nextPrompt = mode === 'written_native_to_target' || mode === 'audio_native_to_target'
        ? nextWord.native
        : nextWord.target;
      prefetchSpeech(nextPrompt, promptLang);
    }
    ...
    >
      ...
      {!hasStarted && words.length > 0 && !gameOver && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-[100] backdrop-blur-sm pointer-events-auto">
          <button 
            onClick={() => {
              warmUpTTS(); // unlock audio + bind the TTS engine on a user gesture
              setHasStarted(true);
            }}
